...
import com.axisframework.eventstore.EventStore;
import com.axisframework.eventstore.EventStoreException;
import com.axisframework.eventstore.EventStreamNotFoundException;
import com.axisframework.eventstore.SnapshotEventStore;
import com.axisframework.eventstore.XStreamEventSerializer;
...
import static com.axisframework.eventstore.fs.EventSerializationUtils.*;

/**
 * Implementation of the {@link EventStore} that serializes objects using XStream and
 ...
 * <p/>
 * Use {@link #setBaseDir(Resource)} to specify the directory where event files should be
 * stored.
 * <p/>
...
public class FileSystemEventStore implements EventStore, SnapshotEventStore {
    ...
    public void appendSnapshotEvent(String type, DomainEvent snapshotEvent) {
        ...
        try {
            ...
            com.axisframework.eventstore.fs.SnapshotEventEntry snapshotEntry = new com.axisframework.eventstore.fs.SnapshotEventEntry(serializedEvent,
                                                                      sequenceNumber,
                                                                      timeStamp,
                                                                      offset);
    ...
    private long calculateOffset(String type, UUID aggregateIdentifier, long sequenceNumber) throws IOException {
        ...
        try {
            ...
            while (lastReadSequenceNumber < sequenceNumber) {
                com.axisframework.eventstore.fs.EventEntry entry = readEventEntry(countingInputStream);
    ...
    private DomainEventStream readEvents(String type, UUID identifier, InputStream eventFileInputStream)
            throws IOException {
        com.axisframework.eventstore.fs.SnapshotEventEntry snapshotEntry = readSnapshotEvent(type, identifier, eventFileInputStream);
    ...
    private com.axisframework.eventstore.fs.SnapshotEventEntry readSnapshotEvent(String type, UUID identifier, InputStream eventFileInputStream)
            throws IOException {
        com.axisframework.eventstore.fs.SnapshotEventEntry snapshotEvent = null;
    ...
    private static class BufferedReaderDomainEventStream implements DomainEventStream {
        ...
        private DomainEvent doReadNext() {
            try {
                com.axisframework.eventstore.fs.EventEntry serializedEvent = readEventEntry(inputStream);
