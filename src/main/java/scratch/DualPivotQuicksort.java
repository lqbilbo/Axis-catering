...
final class DualPivotQuicksort {
    ...
    /**
     * 如果参与排序的数组长度小于这个值，优先考虑插入排序而不是快速排序
     */
    private static final int INSERTION_SORT_THRESHOLD = 47;

    /**
     * 如果参与排序的数组长度大于这个值，优先使用计数排序而不是插入排序
     */
    private static final int COUNTING_SORT_THRESHOLD_FOR_BYTE = 29;

    /**
     * 如果参与排序的短整型或字符数组的长度大于这个值，优先考虑计数排序而不是快速排序
     */
    ...
    /**
     * 如果支持合并，使用给定的工作空间数组分割排序指定的待排序数组
    ...
    static void sort(int[] a, int left, int right,
        ...
        if (run[count] == right++) { // 最后一个有序数列只有最后一个元素
            run[++count] = right;
        } else if (count == 1) { // 整个数组中只有一个有序数列，说明数组已经有序
            return;
        }
